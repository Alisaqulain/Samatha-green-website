  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/95 shadow-sm backdrop-blur">
      <div className="mx-auto flex h-[88px] w-full items-center justify-between gap-4 px-4 sm:px-6 lg:px-8 xl:px-10">
        ...
        <nav ref={navRef} className="hidden items-center gap-5 xl:gap-6 2xl:gap-7 lg:flex">
          {navLinks.map((link) => {
            ...
            return (
              <Link
                key={link.label}
                href={link.href}
                className={`group inline-flex items-center gap-1 text-[0.92rem] font-medium transition-colors ${
                  isActive ? "text-brand-green" : "text-slate-700 hover:text-brand-green"
                }`}
              >
                {link.label}
        ...
        <div className="hidden items-center gap-2.5 lg:flex">
          ...
          <Link
            href="/#download"
            className="inline-flex h-10 items-center gap-2 rounded-full border border-[#b8cce3] bg-white px-4 text-sm font-semibold text-brand-blue transition hover:border-brand-blue hover:bg-[#f5f9fd]"
          >
            <DownloadIcon />
            Download App
          </Link>
          <button
            type="button"
            onClick={() => openQuote()}
            className="inline-flex h-10 items-center rounded-full bg-brand-green px-5 text-sm font-semibold text-white shadow-[0_8px_20px_rgba(45,189,110,0.28)] transition hover:bg-brand-green-dark"
          >
            Get a Free Quote
          </button>
        </div>
