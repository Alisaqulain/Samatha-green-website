    ...
    <section className="bg-[#f4f8fc] px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
      ...
      <div className="mx-auto mt-10 grid max-w-[1200px] gap-6 md:grid-cols-2 xl:grid-cols-3">
        {articles.map((article, index) => (
          <article
            key={article.title}
            className="group flex h-full flex-col rounded-2xl border border-[#E2E8F0] bg-white p-6 shadow-[0_8px_24px_rgba(15,23,42,0.05)] transition hover:-translate-y-1 hover:border-[#2cb673]/45 hover:shadow-[0_16px_34px_rgba(15,23,42,0.1)] animate-fade-up sm:p-7"
            style={{ animationDelay: `${index * 0.12}s` }}
          >
            <span className="flex h-12 w-12 items-center justify-center rounded-xl bg-[#EDF2F7] text-[#125393] transition group-hover:bg-[#2cb673] group-hover:text-white">
              <BookIcon />
            </span>
            <p className="mt-5 text-[12px] font-bold uppercase tracking-[0.16em] text-[#2cb673]">
              {article.category}
            </p>
            <h3 className="mt-3 text-[1.25rem] font-extrabold leading-snug text-[#0A2540] sm:text-[1.35rem]">
              {article.title}
            </h3>
            <p className="mt-3 flex-1 text-[15px] leading-7 text-[#64748B]">{article.excerpt}</p>
            <div className="mt-6 flex items-center gap-2 text-[13px] font-semibold text-[#8a94a6]">
              <CalendarIcon />
